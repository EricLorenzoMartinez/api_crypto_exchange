...
export class TokenHelper {
  static readonly generateToken = (payload: object): string => {
    logger.debug({ payloadSummary: Object.keys(payload) }, 'TokenHelper: Generating token');
  ...
  static readonly verifyToken = (token: string): string | JwtPayload => {
    ...
    try {
      ...
      logger.debug(decoded, 'VERIFY TOKEN: Token decoded successfully');
      logger.debug('TokenHelper: Token verified successfully');
      return decoded;
    } catch (error: unknown) {
      logger.error(error, 'TokenHelper: Token verification failed');
